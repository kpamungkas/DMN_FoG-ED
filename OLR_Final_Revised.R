...
setwd("/Users/kristiapamungkas/Desktop/Master Thesis/Data/Func Connectivity Maps/")
...
## However, it was in-sample evaluation
...
# Empty Model
empty_model <- polr(Outcome ~ 1, trainingset_st, Hess = TRUE)
A <- predict(empty_model)
table(trainingset[, 2], A)

Accuracy(A, trainingset_st$Outcome)
# 67 %
...
# Atlas: AAL3
#
# Sample: Frankenmolle's Cohort (n=10)
...
str(clindata_c3)
names(clindata_c3)
clindata_c3 <- dplyr::select(clindata_c3, ID, Dual_2back_Off, Dual_2back_Clin)
clindata_c3$DualDiff_Clin <- clindata_c3$Dual_2back_Clin - clindata_c3$Dual_2back_Off
clindata_c3$ID <- as.factor(clindata_c3$ID)

# Define Dual_Outcome
sd(clindata_c3$DualDiff_Clin) # 5.986864
clindata_c3$Dual_Outcome <- 0
names(clindata_c3)
for(i in 1:nrow(clindata_c3)){
  a = 0
  if((clindata_c3[i,4] <= 5.986864) & (clindata_c3[i, 4] >= -5.986864)){
    clindata_c3[i,5] = a
  } else if(clindata_c3[i,4] > 5.986864){
    a = 1
    clindata_c3[i, 5] = a
  } else{
    a = -1
...
# Join clinical data with DMN resting-state functional connectivity
names(clindata_c3)
cohort3clin <- dplyr::select(clindata_c3, c(ID, Dual_Outcome))
names(cohort3clin)
cohort3clin[, 3:7] <- rsfc_c3clin
cohort3clin[, 8:11] <- bg_c3clin
cohort3clin[, 12:13] <- motor_c3clin

# Add 1 column of avg DMN and BG RSFC
names(cohort3clin)
cohort3clin$DMN <- rowMeans(cohort3clin[3:7], na.rm = TRUE)
cohort3clin$BG <- rowMeans(cohort3clin[8:11], na.rm = TRUE)
cohort3clin$Motor <- rowMeans(cohort3clin[12:13], na.rm = TRUE)

cohort3clin$Dual_Outcome <- as.factor(cohort3clin$Dual_Outcome)
levels(cohort3clin$Dual_Outcome)
levels(cohort3clin$Dual_Outcome) <- c("Worsening", "Stay", "Improved")


# Subset to DMN, BG, Motor
names(cohort3clin)
cohort3.dmn <- dplyr::select(cohort3clin, ID, Dual_Outcome,
                             FSM, ACC, PCC, AngularGyrus, Precuneus, DMN)
cohort3.bg <- dplyr::select(cohort3clin, ID, Dual_Outcome,
                            Caudate, Putamen, Pallidum, SN, BG)
cohort3.m <- dplyr::select(cohort3clin, ID, Dual_Outcome,
                           M1, SMA, Motor)
...
## Dual Outcome: +- 1 SD (around 5%) changes are grouped to stay group ##

# Create prediction matrices
dmn_pred.transf2 <- model.matrix(lm(Dual_Outcome~ -1 + FSM * PCC * Precuneus, data= cohort3.dmn.st)) 
colnames(dmn_pred.transf2)
cohort3.dmn_transf2 <- dplyr::select(cohort3.dmn.st, Dual_Outcome)
...
h2_2.bg_pred <- predict(mod.bg , cohort3.bg.st[3])
h2_2.bg.prob <- predict(mod.bg, cohort3.bg.st[3], type = "prob")

h2_2.m_pred <- predict(mod.m, cohort3.m.st[3])
h2_2.m.prob <- predict(mod.m, cohort3.m.st[3], type = "prob")
...
Accuracy(h2_2.dmn_pred, cohort3.dmn_transf2$Dual_Outcome)
# 10 %
...
# BG
# Confusion Matrix
table(cohort3.bg.st[, 2], h2_2.bg_pred)
#                    bg_pred
#             Worsening Stay Improved
# Worsening         0    0        1
# Stay              0    0        8
...
Accuracy(h2_2.bg_pred, cohort3.bg.st$Dual_Outcome)
# 10 %
...
# Motor
# Confusion Matrix
table(cohort3.m.st[, 2], h2_2.m_pred)
#                    m_pred
#             Worsening Stay Improved
# Worsening         0    0        1
# Stay              0    0        8
...
Accuracy(h2_2.m_pred, cohort3.m.st$Dual_Outcome)
# 10 %
